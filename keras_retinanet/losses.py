...
def smooth_l1(sigma=3.0, weighting = 1.0):
    ...
    def _smooth_l1(y_true, y_pred):
        ...
        return keras.backend.sum(regression_loss) / normalizer * weighting

    return _smooth_l1


def l1(weighting = 1.0):
    """ Create a  L1 loss functor.
    ...
    def _l1(y_true, y_pred):
        """ Compute the L1 loss of y_pred w.r.t. y_true.
        ...
        Returns
            The L1 loss of y_pred w.r.t. y_true.
        """
        # separate target and state
        regression        = y_pred
        regression_target = y_true[:, :, :-1]
        ...
        normalizer = keras.backend.maximum(1, keras.backend.shape(indices)[0])
        normalizer = keras.backend.cast(normalizer, dtype=keras.backend.floatx())
        return keras.backend.sum(regression_diff) / normalizer * weighting
