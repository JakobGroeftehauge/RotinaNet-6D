...
def default_pose_model(num_values, num_anchors, pyramid_feature_size=256, regression_feature_size=256, name='pose_submodel'):
    ...
    output1 = keras.layers.Reshape((-1, num_values), name='pyramid_regression_reshape')(output1)
    output2 = keras.layers.Reshape((-1, num_values), name='pyramid_regression_reshape')(output2)

    return keras.models.Model(inputs=inputs, outputs=output1, name=name+"1"), keras.models.Model(inputs=inputs, outputs=output2, name=name+"2")
...
def pose_submodels(num_classes, num_anchors):
    ...
    Returns
        A list of tuple, where the first element is the name of the submodel and the second element is the submodel itself.
    """
    rot, trans = default_pose_model(12,num_anchors, name='pose_submodel')
    #rot, trans = layers.SplitLayer()(pose.outputs[0])
    #print("outputs",pose.outputs[0][0])
    print("rot", rot)
    print("def. reg", default_regression_model(4, num_anchors))
    return [
        ('regression', default_regression_model(4, num_anchors)),
        ('classification', default_classification_model(num_classes, num_anchors)),
